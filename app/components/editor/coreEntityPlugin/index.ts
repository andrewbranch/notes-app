...
import { expandEntities } from './steps/expandEntities';

export const createCoreEntityPlugin = (getEditorState: () => EditorState): Plugin => ({
  onChange: editorState => {
    const prevEditorState = getEditorState();
    return expandEntities(collapseEntities(addEntities(removeEntities(editorState, prevEditorState), prevEditorState), prevEditorState));
  },
